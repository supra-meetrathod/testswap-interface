...
const webConfigValues = {
  ...
  awsApiEndpoint:
    process.env.AWS_API_ENDPOINT ?? process.env.REACT_APP_AWS_API_ENDPOINT,

  /** UNISWAP_GATEWAY_DNS — gateway v2 (routing, order status, UniswapX) */
  uniswapGatewayDns:
    process.env.UNISWAP_GATEWAY_DNS ??
  ...
  gitCommitHash:
    process.env.GIT_COMMIT_HASH ?? process.env.REACT_APP_GIT_COMMIT_HASH,
  ...
  disableSourcemap:
    process.env.DISABLE_SOURCEMAP ?? process.env.VITE_DISABLE_SOURCEMAP,
  ...
  analyticsEnabled:
    process.env.ANALYTICS_ENABLED ?? process.env.REACT_APP_ANALYTICS_ENABLED,

  /** SENTRY_ENABLED — gates Sentry error reporting */
  sentryEnabled:
    process.env.SENTRY_ENABLED ?? process.env.REACT_APP_SENTRY_ENABLED,
...
export const webConfigSchema = z.object({
  ...
  webBuildType: z
    .string()
    .default("vite")
    .describe("Web build tool identifier"),
  gitCommitHash: z
    .string()
    .default("")
    .describe("Git commit hash at build time"),
  // API Keys
  walletConnectProjectId: z
    .string()
    .min(1)
    .describe("Project ID for WalletConnect"),
  ...
  entryGatewayApiUrl: z
    .string()
    .optional()
    .describe("URL for entry gateway BFF proxy"),
  websocketUrl: z.string().optional().describe("URL for WebSocket proxy"),
  viteBackendUrl: z
    .string()
    .optional()
    .describe("Override URL for Vite dev proxy target"),

  // Testing & CI
  ci: boolFromString.describe("Is the app running in CI"),
  reportToSlack: boolFromString.describe(
    "Should Playwright results post to Slack"
  ),
  anvilPort: z.coerce
    .number()
    .default(8545)
    .describe("Port for local Anvil blockchain fork"),
  storybookExtension: boolFromString.describe(
    "Is Storybook extension mode enabled"
  ),

  // Build Settings
  enableReactCompiler: boolFromString.describe(
    "Is the React Compiler babel plugin enabled"
  ),
  disableSourcemap: boolFromString.describe("Is sourcemap generation disabled"),
  debugProxy: boolFromString.describe(
    "Is verbose entry-gateway proxy logging enabled"
  ),
  skipCsp: boolFromString.describe("Is CSP meta tag injection skipped"),
  cloudflareEnv: z
    .string()
    .optional()
    .describe("Cloudflare wrangler environment name"),

  // Analytics & Monitoring
  analyticsEnabled: boolFromString.describe(
    "Is remote analytics reporting enabled"
  ),
  sentryEnabled: boolFromString.describe("Is Sentry error reporting enabled"),
  sentryTracesSampleRate: z.coerce
    .number()
    .min(0)
    .max(1)
    .optional()
    .describe("Sentry traces sample rate (0-1)"),
});

export type Config = Omit<
  BaseConfig,
  keyof z.infer<typeof webConfigSchema> & string
> &
  z.infer<typeof webConfigSchema>;
...
export const getConfig = (): Config => {
  ...
  if (
    cachedConfig.environment !== Environment.Production &&
    cachedConfig.nodeEnv !== NodeEnv.Test
  ) {
...
export function getPrivyConfig(isRequired = true): {
  appId: string;
  clientId: string;
} {
  ...
  if (isRequired && (!privyAppId || !privyClientId)) {
    throw new Error(
      "Privy is not configured: PRIVY_APP_ID and PRIVY_CLIENT_ID must be set"
    );
  }
